...
class Spider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        n_plus = response.xpath(
            "//a[@class='n1_climb_4 transition-colors duration-75 hover:text-main inline-block mb-10 sm:mb-5 "
            "font-spectral leading-24']/div/text()").extract()
        ...
        for p in row_data_2:
            scraped_info_2 = {
                'page': response.url,
                'post': p[0].replace(" ", " ").replace("<div>", "").replace("</div>", "").replace
